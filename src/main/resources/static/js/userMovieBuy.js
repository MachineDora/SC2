...
function orderConfirmClick() {
    ...
    postRequest(
        '/ticket/lockSeat',
        {userId:parseInt(sessionStorage.getItem('id')),scheduleId:scheduleId,seats:seats},
        function (res) {
    ...
    var orderInfo = {
        "ticketVOList":res.content.getTicketVOList(),
        "total": 41.0,
        "coupons": [{
            "id": 5,
            "description": "测试优惠券",
...
function postPayRequest() {
    ...
    $('#buyModal').modal('hide')
    var ticketIds = order.ticketId;
    var couponIds = order.couponId;
    if(useVIP){
        postRequest(
            '/ticket/vip/buy?ticketId='+ticketIds+'&couponId='+couponIds,
            null,
            function (res){
                if (res.success){
                    postRequest(
                        '/order/vip/order?ticketId='+ticketIds+'&couponId='+couponIds,
                        null,
                        function (res){
                            if (res.success){
                            }
        ...
        postRequest(
            '/ticket/buy?ticketId='+ticketIds+'&couponId='+couponIds,
            null,
            function (res) {
                if(res.success) {
                    postRequest(
                        '/order/order?ticketId='+ticketIds+'&couponId='+couponIds,
                        null,
                        function (res) {
                            if(res.success) {
                            }
