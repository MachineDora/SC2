...
function repaintScheduleBody(curDateLoc) {
    ...
    for (var i = 0; i < scheduleItems.length; i++) {
        bodyContent += "<tr style='background-color: rgba(255, 255, 255, 0.51)'><td>" + scheduleItems[i].startTime.substring(11, 16) + "</td>" +
...
function renderCommentList() {
    ...
    getRequest(
        ...
        function (res) {
            $('.comment-list').empty();
            var commentStr =" ";
            res.content.forEach(function (comment) {
                commentStr +=
                    "<div class='comIndex'>"+
                    "<div class='userName'>"+
                    comment.name+
            ...
            $('.comment-list').append(commentStr);
        },
        function(error){
            alert(error);
        }
